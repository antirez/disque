...
* `REPLICATE count` is the number of nodes the job should be replicated to. Default: The number of nodes in the cluster or `3` if the cluster is larger than 3 nodes.
* `DELAY sec` is the number of seconds that should elapse before the job is queued by any server. Default: 0 seconds.
* `RETRY sec` period after which, if no `ACKJOB` is received, the job is put again into the queue for delivery. If `RETRY` is 0, the job has at-most-once delivery semantics. Default: `1/10` of the `TTL`.
* `TTL sec` is the max job life in seconds. After this time, the job is deleted even if it was not successfully delivered. Default: `86400` seconds (one day).
